...
export default function Orders() {
  ...
  const [cancellingId, setCancellingId] = useState(null); // order id being cancelled
    ...
    try {
      ...
      if (data.success) {
        ...
        const updated = JSON.parse(localStorage.getItem("customer_orders") || "[]").map(
          (o) => (o.id === cancelTarget.id ? { ...o, status: "Cancelled" } : o)
        );
      ...
      <div className="p-6 xl:p-10">
        ...
        <AnimatePresence>
          {cancelError && (
            ...
            >
              <p className="text-[12px] text-red-500 font-semibold">{cancelError}</p>
              <button onClick={() => setCancelError(null)} className="text-red-300 hover:text-red-500">
                <X size={15} />
              </button>
            </motion.div>
          )}
            ...
            {orders.map((order, idx) => {
              ...
              return (
                ...
                >
                      ...
                      return (
                        <div key={i} className="flex flex-col bg-gray-50 rounded-[24px] p-3">
                          ...
                          <div className="flex flex-wrap gap-1">
                            {sel.drink && (
                              <span className="text-[9px] px-2 py-1 bg-blue-50 text-blue-500 rounded-full">
                                {sel.drink}
                              </span>
                            )}
                            {sel.cake && (
                              <span className="text-[9px] px-2 py-1 bg-yellow-50 text-yellow-600 rounded-full">
                                {sel.cake}
                              </span>
                            )}
                            {sel.extras?.map((e, j) => (
                              <span key={j} className="text-[9px] px-2 py-1 bg-green-50 text-green-600 rounded-full">
                                {e.name}
                              </span>
                            ))}
                          </div>
                  ...
                  {/* STATUS TRACKER — hidden when cancelled */}
                  {!isCancelled ? (
                    <div className="mb-4">
                  ...
                  {isPending && (
                    <button
                      ...
                      className="mt-auto w-full py-3 rounded-[18px] border border-red-200 text-red-400 text-[10px] font-black uppercase tracking-[0.25em] hover:bg-red-50 hover:border-red-300 hover:text-red-500 transition-all active:scale-[0.98] disabled:opacity-40"
                    >
